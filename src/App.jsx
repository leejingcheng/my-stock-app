...
const App = () => {
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `投資損益紀錄_${marketTab}_${selectedYear}年.csv`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const recordExchangeRate = formData.market === 'Overseas' ? (parseFloat(formData.exchangeRate) || exchangeRate) : 1;

      const recordData = { ...formData, shares, cost, revenue, profit, profitRate, exchangeRate: recordExchangeRate };
      ...
      console.error(err);
      setErrorMsg('儲存失敗：請檢查 Firebase Firestore Database 的 Rules 規則是否已設為 allow read, write: if true;');
    }
  };
  ...
  const theme = isDarkMode ? {
    ...
    textMain: "text-slate-200", textMuted: "text-slate-400", textStrong: "text-white",
    borderMain: "border-white/5", backdrop: "bg-black/80"
  } : {
    bgMain: "bg-slate-50", bgCard: "bg-white", bgInner: "bg-slate-50", bgInput: "bg-slate-100",
    textMain: "text-slate-800", textMuted: "text-slate-500", textStrong: "text-slate-900",
    borderMain: "border-slate-200", backdrop: "bg-slate-900/40"
  };
  ...
  return (
    <div className={`min-h-screen ${theme.bgMain} ${theme.textMain} font-sans pb-8 transition-colors duration-300`}>
            ...
            {['總覽', '台股', '海外股票'].map(tab => {
              ...
              return (
                <button key={tab} onClick={() => setMarketTab(tabValue)} className={`pb-2 relative whitespace-nowrap ${isActive ? 'text-emerald-500' : theme.textMuted}`}>
                  {tab}{isActive && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-1 bg-emerald-500 rounded-full" />}
                </button>
              );
            })}
          </div>
          <div className="pb-2">
            <div className={`flex items-center gap-1 text-sm font-bold ${theme.textStrong}`}>
              <Filter size={14} className={theme.textMuted} />
              <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="bg-transparent outline-none cursor-pointer appearance-none text-right">
      ...
      <main className="p-4 space-y-4">
          ...
          <div className="space-y-4">
            <div className={`${theme.bgCard} p-5 rounded-2xl border ${theme.borderMain} shadow-sm`}>
              <div className="flex justify-between items-center mb-4">
                ...
                <div className={`text-xs flex items-center gap-1 ${theme.textMuted} ${isDarkMode ? 'bg-black/20' : 'bg-slate-100'} px-2 py-1 rounded-lg`}>
                  <span>預設匯率 =</span>
                  <input type="number" step="0.1" value={exchangeRate} onChange={(e) => setExchangeRate(Number(e.target.value))} className="w-12 bg-transparent border-b border-emerald-500 outline-none text-center text-emerald-500 font-bold" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className={`p-3 rounded-xl ${theme.bgInner} border ${theme.borderMain}`}>
                   <p className="text-[10px] uppercase font-bold text-slate-500 mb-1">總成本</p>
                   <p className={`font-mono font-bold ${theme.textStrong}`}>NT$ {summaryStats.cost.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                </div>
                <div className={`p-3 rounded-xl ${theme.bgInner} border ${theme.borderMain}`}>
                   <p className="text-[10px] uppercase font-bold text-slate-500 mb-1">總收入</p>
                   <p className={`font-mono font-bold ${theme.textStrong}`}>NT$ {summaryStats.revenue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                </div>
              </div>
            </div>
            ...
            {chartData.assetProfits.length > 0 && (
              <div className={`${theme.bgCard} p-5 rounded-2xl border ${theme.borderMain}`}>
                <h3 className={`font-bold ${theme.textStrong} mb-6 flex items-center gap-2`}><BarChart2 size={18} className="text-emerald-500" />累積損益排行</h3>
                <div className="space-y-5">
                  {chartData.assetProfits.slice(0, isRankingExpanded ? undefined : 5).map(asset => {
                    ...
                    return (
                      <div key={asset.symbol} className="flex flex-col gap-1.5">
                        <div className="flex justify-between text-xs px-1">
                          <span className={`font-bold ${theme.textStrong}`}>{asset.symbol} <span className="font-normal opacity-50 ml-1">{asset.name}</span></span>
                          <span className={`font-mono ${getPriceColor(asset.profit)}`}>{asset.profit >= 0 ? '+' : ''}{asset.profit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                        </div>
                  ...
                  {chartData.assetProfits.length > 5 && (
                    <button onClick={() => setIsRankingExpanded(!isRankingExpanded)} className={`w-full pt-4 flex items-center justify-center gap-1 text-xs font-medium ${theme.textMuted} border-t ${theme.borderMain}`}>
                      {isRankingExpanded ? <>收起 <ChevronUp size={14} /></> : <>展開全部 <ChevronDown size={14} /></>}
                    </button>
                  )}
                </div>
              </div>
        ...
        {marketTab !== 'Overview' && (
          <div className="space-y-6">
            {Object.entries(visibleRecords.reduce((acc, r) => {
              const y = r.date.substring(0, 4);
              if (!acc[y]) acc[y] = [];
              acc[y].push(r);
              return acc;
            }, {})).sort(([a], [b]) => b - a).map(([year, yrRecs]) => (
              <div key={year}>
                <button onClick={() => setCollapsedYears(p => ({...p, [year]: !p[year]}))} className={`w-full py-2 -mx-4 px-4 border-y ${theme.borderMain} ${isDarkMode ? 'bg-white/5' : 'bg-slate-100'} text-xs font-mono tracking-widest ${theme.textMuted} flex justify-center items-center`}>
                  {year}年 {collapsedYears[year] ? <ChevronDown size={14} className="ml-2" /> : <ChevronUp size={14} className="ml-2" />}
                </button>
                {!collapsedYears[year] && (
                  <div className="space-y-4 mt-4 animate-in fade-in slide-in-from-top-2 duration-300">
                    {yrRecs.map(r => {
                      const isDiv = r.recordType === 'dividend';
                      return (
                        <div key={r.id} className={`${theme.bgCard} rounded-xl overflow-hidden shadow-sm border ${theme.borderMain} relative transition-all active:scale-[0.99]`}>
                          {isDiv && <div className="absolute left-0 top-0 bottom-0 w-1 bg-yellow-500" />}
                          <div className="p-4">
                            <div className="flex justify-between items-start mb-1">
                              <div>
                                <div className="flex items-center gap-2">
                                   <span className={`${theme.textMuted} text-sm font-bold`}>{r.symbol}</span>
                                   <span className={`text-[10px] px-1.5 py-0.5 rounded border ${r.market === 'TW' ? 'border-blue-500/30 text-blue-500' : 'border-purple-500/30 text-purple-500'}`}>{r.market === 'TW' ? '台股' : '美股'}</span>
                                </div>
                                <h3 className={`${theme.textStrong} font-bold text-lg leading-tight mt-1`}>{r.name}</h3>
                              </div>
                              <span className={`${theme.textMuted} text-xs`}>{r.date.slice(5).replace('-', '/')}</span>
                            </div>
                            <div className="flex justify-between items-end mt-4">
                              <div className="space-y-1">
                                <div className="flex items-center gap-2 text-sm"><span className={theme.textMuted}>{isDiv ? '類型' : '股數'}</span><span className={theme.textStrong}>{isDiv ? '配息' : r.shares}</span></div>
                                <div className="flex items-center gap-2 text-sm"><span className={theme.textMuted}>{isDiv ? '實收' : '損益'}</span><span className={`font-bold ${getPriceColor(r.profit)}`}>{r.profit >= 0 ? '+' : ''}{r.profit.toLocaleString(undefined, { maximumFractionDigits: 2 })}{!isDiv && ` (${r.profitRate >= 0 ? '+' : ''}${r.profitRate.toFixed(2)}%)`}</span></div>
                              </div>
                              <div className="flex gap-1 -mr-2">
                                <button onClick={() => handleEditClick(r)} className="text-slate-400 p-2"><Edit2 size={16} /></button>
                                <button onClick={() => handleDelete(r.id)} className="text-slate-400 p-2"><Trash2 size={16} /></button>
                              </div>
                            </div>
                          </div>
                          <div className={`${theme.bgInner} p-4 space-y-2 border-t ${theme.borderMain} text-xs`}>
                            {r.market === 'Overseas' && <div className="flex justify-between"><span className={theme.textMuted}>結匯匯率</span><span className={theme.textStrong}>{r.exchangeRate}</span></div>}
                            <div className="flex justify-between"><span className={theme.textMuted}>{isDiv ? '配息總額' : '成交總額'}</span><span className={theme.textStrong}>{r.revenue.toLocaleString()}</span></div>
                            {!isDiv && <div className="flex justify-between"><span className={theme.textMuted}>投資成本</span><span className={theme.textStrong}>{r.cost.toLocaleString()}</span></div>}
                          </div>
                        </div>
                      );
                    })}
      ...
      <button onClick={() => { setEditingId(null); setIsModalOpen(true); }} className="fixed bottom-8 right-6 w-14 h-14 bg-emerald-500 text-white rounded-full shadow-lg flex items-center justify-center active:scale-95 transition-all z-30">
        ...
        <div className={`fixed inset-0 ${theme.backdrop} backdrop-blur-sm z-50 flex items-center justify-center p-4`}>
          <div className={`${theme.bgCard} w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]`}>
            <div className="p-4 border-b border-white/5 flex justify-between items-center bg-black/5">
              <h3 className="font-bold">紀錄細節</h3>
              <button onClick={closeAndResetModal} className={theme.textMuted}>關閉</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-2">
                {['TW', 'Overseas'].map(m => (
                  <button key={m} type="button" onClick={() => setFormData({...formData, market: m})} className={`py-2 rounded-lg text-sm font-bold border transition-all ${formData.market === m ? 'border-emerald-500 bg-emerald-500/10 text-emerald-500' : 'border-white/5 text-slate-500'}`}>{m === 'TW' ? '台股' : '海外'}</button>
                ))}
              </div>
              <div className="flex rounded-lg p-1 bg-black/10">
                {['trade', 'dividend'].map(t => (
                  <button key={t} type="button" onClick={() => setFormData({...formData, recordType: t})} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${formData.recordType === t ? (t === 'trade' ? 'bg-emerald-500' : 'bg-yellow-500') + ' text-white shadow' : 'text-slate-500'}`}>{t === 'trade' ? '交易' : '配息'}</button>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-2">
              ...
              {formData.market === 'Overseas' && (
                <div className="space-y-1">
                  <div className="flex justify-between text-[10px] font-bold text-slate-500 px-1"><span>匯率</span>{isFetchingRate && <RefreshCw size={10} className="animate-spin text-emerald-500" />}</div>
                  <input type="number" step="0.01" required className={`${theme.bgInput} w-full rounded-lg p-3 outline-none`} value={formData.exchangeRate} onChange={e => setFormData({...formData, exchangeRate: e.target.value})} />
                </div>
              )}
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-500 px-1">實收 / 賣出總額</span>
                <input type="number" step="0.01" required className={`${theme.bgInput} w-full rounded-lg p-3 outline-none text-xl font-mono`} value={formData.revenue} onChange={e => setFormData({...formData, revenue: e.target.value})} />
              </div>
              {formData.recordType === 'trade' && (
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-500 px-1">總投資成本</span>
                  <input type="number" step="0.01" required className={`${theme.bgInput} w-full rounded-lg p-3 outline-none text-xl font-mono`} value={formData.cost} onChange={e => setFormData({...formData, cost: e.target.value})} />
                </div>
              )}
              <button type="submit" className={`w-full text-white font-bold py-4 rounded-xl shadow-lg transition-all active:scale-95 ${editingId ? 'bg-blue-500' : 'bg-emerald-500'}`}>{editingId ? '確認修改' : '儲存紀錄'}</button>
            </form>
          </div>
        </div>
      )}
